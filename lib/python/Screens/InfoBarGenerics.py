...
class InfoBarShowHide(InfoBarScreenSaver):
    ...
    def doHide(self):
        if self.__state == self.STATE_SHOWN:
            if self.dimmed > 0:
                self.doWriteAlpha(config.av.osd_alpha.value * self.dimmed / 25)
    ...
    def inactivityTimeout(self):
        if config.usage.inactivity_timer_blocktime.value:
            ...
            if curtime.tm_year > 1970:
                ...
                if begintime <= endtime and curtime >= begintime and curtime < endtime or begintime > endtime and (curtime >= begintime or curtime < endtime) or config.usage.inactivity_timer_blocktime_extra.value and (begintime_extra <= endtime_extra and curtime >= begintime_extra and curtime < endtime_extra or begintime_extra > endtime_extra and (curtime >= begintime_extra or curtime < endtime_extra)):
